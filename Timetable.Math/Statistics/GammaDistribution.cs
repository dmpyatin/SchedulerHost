...
namespace Timetable.Math.Statistics
{
    ...
    public class GammaDistribution : ProbabilityDensityFunction
    {
        ...
        {
            return -System.Math.Log(1 - Generator.NextDouble());
        }
        ...
        private double RandomForAlphaGreaterThan1()
        {
            ...
            while (true)
            {
                ...
                v = _a * System.Math.Log(u1 / (1 - u1));
                y = _alpha * System.Math.Exp(v);
                z = u1 * u1 * u2;
                w = _b + _q * v - y;
                if (w + _d - 4.5 * z >= 0 || w >= System.Math.Log(z))
                    return y;
            }
        }
        ...
        private double RandomForAlphaLessThan1()
        {
            ...
            while (true)
            {
                ...
                if (p > 1)
                {
                    y = -System.Math.Log((_b - p) / _alpha);
                    if (Generator.NextDouble() <= System.Math.Pow(y, _alpha - 1))
                        return y;
                }
                y = System.Math.Pow(p, 1 / _alpha);
                if (Generator.NextDouble() <= System.Math.Exp(-y))
                    return y;
            }
        }
        ...
        {
            get { return 2 / System.Math.Sqrt(_alpha); }
        }
        ...
        {
            return x > 0 ? System.Math.Exp(System.Math.Log(x) * (_alpha - 1) - x / _beta - _norm) : 0;
        }
