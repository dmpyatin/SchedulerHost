...
namespace Timetable.Math.Statistics
{
    ...
    public sealed class LaplaceDistribution : ProbabilityDensityFunction
    {
        ...
        {
            return x > _mu
                    ? 1 - System.Math.Exp(-(x - _mu) / _beta) / 2
                    : System.Math.Exp(-(x - _mu) / _beta) / 2;
        }
        ...
        {
            return x < 0.5
                    ? _mu + _beta * System.Math.Log(2 * x)
                    : _mu - _beta * System.Math.Log(2 - 2 * x);
        }

        /// @return double a random number distributed according to the receiver.
        public override double Random()
        {
            double r = -_beta * System.Math.Log(this.Generator.NextDouble());
        ...
        {
            get { return _beta / System.Math.Sqrt(2); }
        }
        ...
        {
            return System.Math.Exp(-System.Math.Abs(x - _mu) / _beta) / (2 * _beta);
        }
