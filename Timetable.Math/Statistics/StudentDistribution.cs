...
namespace Timetable.Math.Statistics
{
    ...
    public sealed class StudentDistribution : ProbabilityDensityFunction
    {
        ...
        public StudentDistribution(int n)
        {
            if (n <= 0)
                throw new ArgumentOutOfRangeException(
                                        Resource.Degree_of_freedom_must_be_positive);
            DefineParameters(n);
        }
        ...
        {
            _dof = n;
            _norm = -(System.Math.Log(_dof) * 0.5
        ...
        public override double DistributionValue(double x)
        {
            if (x == 0)
                return 0.5;
            double acc = SymmetricAcceptance(System.Math.Abs(x));
        ...
        public override double[] Parameters
        {
            ...
            set { DefineParameters((int)System.Math.Round(value[0])); }
        }
        ...
        public override double Random()
        {
            ...
            return Generator.NextGaussian() * System.Math.Sqrt((_dof - 1) / _chiSquareDistribution.Random());
        }
        ...
        {
            return System.Math.Exp(_norm - System.Math.Log(x * x / _dof + 1) * (_dof + 1) / 2);
        }
