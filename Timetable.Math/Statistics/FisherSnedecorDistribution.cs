...
namespace Timetable.Math.Statistics
{
    ...
    public sealed class FisherSnedecorDistribution: ProbabilityDensityFunction
    {
        ...
        /// Create a new instance of the Fisher-Snedecor distribution with given degrees of freedom.
        ...
        public FisherSnedecorDistribution(int n1, int n2)
        {
            if (n1 <= 0)
                throw new ArgumentOutOfRangeException(
                                Resource.First_degree_of_freedom_must_be_positive);
            if (n2 <= 0)
                throw new ArgumentOutOfRangeException(
                                Resource.Second_degree_of_freedom_must_be_positive);
        ...
        public override double Average
        {
            ...
            set { throw new InvalidOperationException(Resource.Set_not_defined_for_FisherSnedecorDistribution_Average); }
        }
        ...
        {
            if (x <= 0)
                throw new ArgumentOutOfRangeException(Resource.F_CL_argument_must_be_positive);
        ...
        public override double[] Parameters
        {
            ...
            set
            {
                DefineParameters((int)System.Math.Round(value[0]), (int)System.Math.Round(value[1]));
            }
        }
        ...
        {
            return x > 0
                    ? System.Math.Exp(_norm + System.Math.Log(x) * (_dof1 / 2 - 1)
                                    - System.Math.Log(x * _dof1 + _dof2)
                                                    * ((_dof1 + _dof2) / 2))
                    : 0;
        }
        ...
        public override double Variance
        {
            ...
            {
                return _dof2 > 4
                        ? _dof2 * _dof2 * 2 * (_dof1 + _dof2 + 2) / (_dof1 * (_dof2 - 2) * (_dof2 - 4))
                        : double.NaN;
            }
        }
    }
