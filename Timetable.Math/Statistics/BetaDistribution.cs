...
namespace Timetable.Math.Statistics
{
    ...
    public sealed class BetaDistribution : ProbabilityDensityFunction
    {
        ...
        public override double Skewness
        {
            get
            {
                return 2 * System.Math.Sqrt(_alpha1 + _alpha2 + 1) * (_alpha2 - _alpha1)
                                / (System.Math.Sqrt(_alpha1 * _alpha2)
                                                        * (_alpha1 + _alpha2 + 2));
            }
        }
        ...
        {
            return System.Math.Exp(System.Math.Log(x) * (_alpha1 - 1)
                                + System.Math.Log(1 - x) * (_alpha2 - 1) - _norm);
        }
