namespace Timetable.Math.Interpolation
{
    ...
    *************************************************************************/
        public BarycentricInterpolation(double[] x,double[] f,double[] w,int n,double t)
        {
            ...
            threshold = System.Math.Sqrt(Timetable.Math.Math.MinRealNumber);
            //
            ...
            for (i = 1; i <= n - 1; i++)
            {
                if (System.Math.Abs(t - x[i]) < System.Math.Abs(s))
                {
                    s = t - x[i];
                    j = i;
                }
            ...
            if (System.Math.Abs(s) > threshold)
            {
