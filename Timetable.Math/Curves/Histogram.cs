...
using Timetable.Math.Regression;
using Timetable.Math.Statistics;
...
namespace Timetable.Math.Curves
{
    ...
    public class Histogram : IPointSeries, IHistogram
    {
        ...
        {
            return (int)System.Math.Floor((x - _minimum) / _binWidth);
        }
        ...
        private void DefineParameters(double from, double to, int bins)
        {
            ...
            _binWidth = DhbMath.RoundToScale((to - from) / bins,
                                                            _integerBinWidth);
            _minimum = _binWidth * System.Math.Floor(from / _binWidth);
            int numberOfBins = (int)System.Math.Ceiling((to - _minimum) / _binWidth);
        ...
        {
            int addSize = (int)System.Math.Ceiling((_minimum - x) / _binWidth);
...
//                newContents[n] = 0;
        ...
        {
            int newSize = (int)System.Math.Ceiling((x - _minimum) / _binWidth);
...
//            newContents[n] = 1;
        ...
        public double[] GetBinParameters(double x)
        {
            if (x >= _minimum)
            {
                int index = (int)System.Math.Floor((x - _minimum) / _binWidth);
                if (index < _contents.Length)
                {
        ...
        public double[] Range
        {
            get
            {
                ...
                for (int n = 0; n < _contents.Length; n++)
                    range[3] = System.Math.Max(range[3], _contents[n]);
                return range;
            }
        }
