    ...
    public class JacobiTransformation : IterativeProcess
    {
        ...
        public override void FinalizeIterations()
        {
            ...
            while (bound >= 0)
            {
                ...
                for (i = 0; i < bound; i++)
                {
                    if (System.Math.Abs(_rows[i, i]) < System.Math.Abs(_rows[i + 1, i + 1]))
                    {
                        Exchange(i);
                        m = i;
                    }
        ...
        private void Transform()
        {
            ...
            double t = arp > 0 ? 1 / (System.Math.Sqrt(arp * arp + 1) + arp)
                               : 1 / (arp - System.Math.Sqrt(arp * arp + 1));
